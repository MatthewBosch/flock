...
def train_lora(model_id: str, context_length: int, training_args: LoraTrainingArguments):
    ...
    if model_id == "microsoft/Phi-3.5-mini-instruct":
        # Modify target modules based on your inspection of the model parameters
        target_modules = ["qkv_proj"]  # Example: change this to the correct module(s) you find
    else:
        target_modules = ["q_proj", "v_proj"]  # Default for models like Qwen

    lora_config = LoraConfig(
        r=training_args.lora_rank,
        target_modules=target_modules,  # Use the correct target modules
